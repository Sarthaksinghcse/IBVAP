...
# Verbatim copy of ai_engine/intelligence/anpr_engine.py at commit e13d158 ("baseline").
# Kept so that `run_bench.py --engine current` stays runnable after Task 6 rewrites
# the live engine in place, making the before/after table in Task 9 reproducible.
# Path depth matches the original (repo/x/y/file.py), so its models_dir default
# still resolves to repo/models.
