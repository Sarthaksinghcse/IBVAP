...
def run_all_tests():
    ...
    success, emb, err, _ = engine_inst.process_registration_image(non_face_img)
    ...
    try:
        ...
        is_match, pid, pname, ident, priority, sim_pct, cosine_score, _ = engine_inst.match_against_watchlist(
        ...
        is_match_b, pid_b, pname_b, _, _, sim_pct_b, cosine_b, _ = engine_inst.match_against_watchlist(
        ...
        is_strict_match, _, _, _, _, _, _, _ = engine_inst.match_against_watchlist(
            query_same, watchlist_records, threshold=0.9999
        )
        print(f"  Strict threshold (0.9999) -> match: {is_strict_match} (Strict enforcement)")

        is_std_match, _, _, _, _, _, _, _ = engine_inst.match_against_watchlist(
            query_same, watchlist_records, threshold=0.50
        )
