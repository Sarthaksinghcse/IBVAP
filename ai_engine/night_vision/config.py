...
class NightVisionConfig:
    ...
    Defaults incorporate benchmark-tuned noise suppression parameters:
      - Bilateral filter: diameter=9, sigma=75.0 (dominates noise suppression)
      - Sharpening: gentle amount=0.25 (prevents sharpening noise into false-positive animal detections)
    """
    ...
    # destabilises both the tracker and detection confidence scores.
    ...
    # ── Denoising ─────────────────────────────────────────────────────────────
    # Tuned parameters: d=9, sigma=75.0 provides essential noise suppression
    # under sensor gain amplification.
    ...
    # Tuned to gentle 0.25 to prevent sharpening residual sensor noise.
