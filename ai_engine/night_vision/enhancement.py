...
--------
    ...
    Gamma correction        (LUT, brightens midtones)
         │
    Denoising               (bilateral d=9 σ=75; NLM opt-in for offline work)
         │
    Unsharp-mask sharpening (gentle 0.25, recovers edges softened by denoising)
         │
    Enhanced BGR frame
...
class NightVisionEnhancer:
    """
    Per-stream low-light frame enhancer.

    Maintains the state needed to make temporal decisions:
      - lighting measurement cache (sampled every Nth frame for low overhead)
      - hysteresis state (prevents flapping at the boundary)
      - runtime telemetry (frames seen, frames enhanced, cumulative runtime)
    """

    def __init__(self, config: Optional[NightVisionConfig] = None):
        self.config = config or NightVisionConfig()

        # Cached OpenCV primitives so we do not reallocate per frame.
        ...
        self._engaged: bool = False  # hysteresis state

        # Telemetry
        self.frames_seen: int = 0
        self.frames_enhanced: int = 0
        self.total_enhance_ms: float = 0.0

    # ── Lighting measurement ──────────────────────────────────────────────────

    def _measure_lighting(self, frame: np.ndarray) -> FrameLighting:
        """
        Compute mean luminance, contrast, and dark-pixel ratio on a thumbnail.

        Downscaling to 1/8th keeps the measurement well under 1ms while
        producing a statistically identical result to a full-frame pass.
        """
        scale = max(1, self.config.luminance_downscale)
        if scale > 1 and frame.shape[0] > scale * 8 and frame.shape[1] > scale * 8:
            thumb = cv2.resize(
                frame,
        ...
        dark_ratio = float(np.count_nonzero(l_channel < 40) / l_channel.size)

        cfg = self.config
        if mean_l < cfg.extreme_low_threshold:
            profile = PROFILE_EXTREME_LOW
        elif mean_l < cfg.night_threshold:
            profile = PROFILE_NIGHT
        elif mean_l < cfg.dusk_threshold:
            profile = PROFILE_DUSK
        else:
            profile = PROFILE_DAY

        return FrameLighting(mean_l, std_l, dark_ratio, profile)

    def _lighting_for_frame(self, frame: np.ndarray, force: bool = False) -> FrameLighting:
        """Sample luminance every Nth frame, re-using cached state in between."""
        interval = max(1, self.config.luminance_sample_interval)
        if force or self._cached_lighting is None or (self._frame_counter % interval) == 0:
            self._cached_lighting = self._measure_lighting(frame)
    ...
    def _should_enhance(self, lighting: FrameLighting) -> Tuple[bool, str]:
        """
        Decide whether to enhance the current frame.

        Applies the configured mode and the hysteresis band so the system never
        flaps between enhanced and raw on borderline lighting.
        """
        cfg = self.config

        if cfg.mode == MODE_OFF:
            self._engaged = False
            return False, "Night vision mode is OFF; frame passed through unmodified."

        if cfg.mode == MODE_ALWAYS:
            self._engaged = True
            return True, f"Night vision mode is ALWAYS; enhancement forced ({lighting.profile})."

        # MODE_AUTO — engage when dark, stay engaged until safely above threshold.
        if self._engaged:
    ...
    def _apply_enhancement(self, frame: np.ndarray, profile: str) -> np.ndarray:
        ...
        # 2. Gamma correction to lift midtones (LUT — one pass, no per-pixel float math).
        ...
        if self._denoise_applies_to(profile):
            if cfg.denoise_method == "BILATERAL":
                ...
                # High quality, very slow. Offline / forensic use only.
                out = cv2.fastNlMeansDenoisingColored(out, None, 5, 5, 7, 21)

        # 4. Unsharp mask to restore edges softened by denoising.
    ...
    def process(self, frame: np.ndarray, force_measure: bool = False) -> EnhancementResult:
        """
        ...
        lighting = self._lighting_for_frame(frame, force=force_measure)
    ...
    def enhance(self, frame: np.ndarray, force_measure: bool = False) -> np.ndarray:
        """Convenience wrapper returning just the frame, for drop-in call sites."""
        return self.process(frame, force_measure=force_measure).frame
