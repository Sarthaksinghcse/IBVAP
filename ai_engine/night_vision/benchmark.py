...
from ai_engine.night_vision.config import NightVisionConfig, MODE_ALWAYS
...
def _warmup(detector: Detector, source: str, frames: int = 5) -> None:
    """Warm up PyTorch / YOLO kernels so first-inference cost does not skew FPS."""
    cap = cv2.VideoCapture(source)
    if not cap.isOpened():
        return
...
def run_benchmark(
    ...
    # Warm up the model before either timed pass.
    ...
    # Check for noise amplification failure mode (Section 2 finding)
    b_pv = b["per_class"].get("PERSON", 0) + b["per_class"].get("VEHICLE", 0)
    e_pv = e["per_class"].get("PERSON", 0) + e["per_class"].get("VEHICLE", 0)
    b_animal = b["per_class"].get("ANIMAL", 0)
    e_animal = e["per_class"].get("ANIMAL", 0)
    noise_amplification = False
    if (e_animal > b_animal) and (e_pv <= b_pv):
        noise_amplification = True
        logger.warning("\n[WARNING] SENSOR NOISE AMPLIFICATION DETECTED!")
        logger.warning("Detection gain is concentrated in ANIMAL false-positives while PERSON/VEHICLE are flat or reduced.")
        logger.warning("Recommended: increase bilateral denoise or reduce sharpen_amount.\n")

    verdict = _verdict(b, e, noise_amplification)
    logger.info(f"\nVERDICT: {verdict}\n")
...
def _verdict(b: Dict[str, Any], e: Dict[str, Any], noise_amplification: bool = False) -> str:
    """State plainly whether enhancement helped, hurt, or did nothing measurable."""
    if noise_amplification:
        return "Enhancement DEGRADED quality due to noise amplification (ANIMAL false positives). Needs parameter tuning."
